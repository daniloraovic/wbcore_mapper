...
class My_Model_Mapper_Mongo extends My_Model_Mapper
{
    ...
    public function update_upsert($obj)
    {
        ...
        return $collection->update(array(
            $this->_upsert_key => $data[$this->_upsert_key]
        ), array(
            '$set' => $data
        ), array(
            'upsert' => true
    ...
    public function delete($obj)
    {
        ...
        if ($obj instanceof My_Model_Domain) {
            ...
            return $collection->remove($this->_getSelection()
                ->where($obj));
        } else {
    ...
    protected function _selectAll(My_Model_Mapper_Mongo_IdentityObject $identity)
    {
        ...
        $cursor = $collection->find($this->_getSelection()
            ->where($identity));
        
        $cursor->sort($this->_getSelection()
            ->orderBy($identity));
        
        $cursor->limit($this->_getSelection()
            ->limit($identity));
        
        $cursor->skip($this->_getSelection()
            ->offset($identity));
    ...
    protected function _selectOne(My_Model_Mapper_Mongo_IdentityObject $identity)
    {
        ...
        return $collection->findOne($this->_getSelection()
            ->where($identity));
    }
    ...
    public function count(My_Model_Mapper_Mongo_IdentityObject $identity)
    {
        ...
        return $collection->count($this->_getSelection()
            ->where($identity));
    }
